	...
	Monster* MonsterFactory::CreateMonster(eMonsterType eType, Vector2 pos, Player* pPlayer, ExpGemObjPool* pExpGemObejctPool, MonsterObjPool<Monster>* pMonsterObjPool)
	{
		...
		case eMonsterType::MUDMAN:
		{
			...
			pMonster = pMonsterObjPool->Get(
				...
				L"Resources\\Image\\MudmanRHittedAnim.bmp",
				L"Resources\\Image\\MudmanLHittedAnim.bmp",
				pExpGemObejctPool,
				pMonsterObjPool
			);
		...
		case eMonsterType::GREEN_GHOST:
		{
			...
			pMonster = pMonsterObjPool->Get(
				...
				L"Resources\\Image\\GreenGhostRightHittedAnim.bmp",
				L"Resources\\Image\\GreenGhostLeftHittedAnim.bmp",
				pExpGemObejctPool,
				pMonsterObjPool
			);
			Scene* scene = SceneManager::GetSpecifiedScene(eSceneType::PLAY_SCENE);
		...
		case eMonsterType::MEDUSA_HEAD:
		{
			...
			pMonster = pMonsterObjPool->Get(
				...
				L"Resources\\Image\\MedusaHeadRightHittedAnim.bmp",
				L"Resources\\Image\\MedusaHeadLeftHittedAnim.bmp",
				pExpGemObejctPool,
				pMonsterObjPool
			);
			Scene* scene = SceneManager::GetSpecifiedScene(eSceneType::PLAY_SCENE);
